                ...
                <form action="{{ route('orders.update', $order->id) }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="status" class="form-control" id="orderStatus">
                                    <option value="Awaiting Confirmation" @selected($order->status == 'Awaiting
                                        Confirmation')>Awaiting Confirmation</option>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="payment_status" class="form-control" id="paymentStatus">
                                    ...
                                    <option value="Refunded" @selected($order->payment_status == 'Refunded')>Refunded
                                    </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-12 mt-3">
                            <x-form-input name="order_update" label="Order Update" value="{{ $order->update }}">
                            </x-form-input>
                        </div>

                        @if(!$order->user->vendor)
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="assignedTo">Assigned To</label>
                                <select name="assigned_to" class="form-control chosen" id="assignedTo">
                                    <option value="" @selected(!$order->assigned_to)>Self</option>
                                    @foreach ($vendors as $vendor)
                                    <option value="{{ $vendor->id }}" @selected($order->assigned_to==$vendor->id)>{{
                                        $vendor->name }}</option>
                                    @endforeach
                                </select>
                            </div>
...
<x-include-plugins :plugins="['chosen']" />
